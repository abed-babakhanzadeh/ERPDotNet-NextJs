...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const errorMessage = error.response?.data || "نام کاربری یا رمز عبور اشتباه است";
      toast.error("خطای ورود", {
        description: typeof errorMessage === 'string' ? errorMessage : "ارتباط با سرور برقرار نشد"
      });
    } finally {
      setLoading(false);
    }
    ...
    <div className="flex min-h-screen items-center justify-center bg-gray-100 p-4">
      <div className="w-full max-w-md overflow-hidden rounded-2xl bg-white shadow-xl border border-gray-200">
        ...
        <div className="bg-blue-700 p-8 text-center">
          ...
          <p className="mt-2 text-sm text-blue-100">لطفاً مشخصات کاربری خود را وارد کنید</p>
        </div>
        ...
        <div className="bg-gray-50 p-4 text-center border-t border-gray-100">
          <p className="text-xs text-gray-500">نسخه ۱۰.۰ - توسعه داده شده با .NET & Next.js</p>
        </div>
      </div>
    </div>
  );
