...
function FilterPopoverContent({ column, initialFilter, onApply, onClear }: { 
    column: ColumnConfig;
    initialFilter: ColumnFilter | undefined,
    onApply: (filter: ColumnFilter) => void;
    onClear: () => void;
}) {
    const defaultCondition: FilterCondition = { id: crypto.randomUUID(), operator: filterOperators[column.type][0].value, value: ''};
    const [logic, setLogic] = useState<'and' | 'or'>(initialFilter?.logic || 'and');
    const [conditions, setConditions] = useState<FilterCondition[]>(initialFilter?.conditions.length ? initialFilter.conditions : [defaultCondition]);
    ...
    const addCondition = () => {
        setConditions(prev => [...prev, { id: crypto.randomUUID(), operator: filterOperators[column.type][0].value, value: '' }]);
    };
    ...
    return (
        <div className="grid gap-4" onKeyDown={handleKeyDown}>
            ...
            {conditions.map((condition, index) => (
                <div key={condition.id} className="grid gap-2 p-2 border rounded-md relative">
                     {conditions.length > 1 && (
                        <Button variant="ghost" size="icon" className="absolute top-1 left-1 h-6 w-6 text-muted-foreground hover:text-destructive" onClick={() => removeCondition(condition.id)}>
                    ...
                    <div className="grid grid-cols-2 gap-2">
                        <div className="grid gap-2">
                            ...
                            <Select value={condition.operator} onValueChange={op => updateCondition(condition.id, { operator: op })}>
                         ...
                         <div className="grid gap-2">
                            ...
                            <FilterInput columnType={column.type} condition={condition} onChange={updateCondition} />
                        </div>
                    </div>
                     {condition.operator === 'between' && (
                        <div className="grid grid-cols-2 gap-2">
                            ...
                            <div className="grid gap-2">
                                 ...
                                 <Input id={`value2-${condition.id}`} value={condition.value2 ?? ''} onChange={(e) => updateCondition(condition.id, { value2: e.target.value })} type='number' />
                            </div>
                        </div>
                    )}
                </div>
            ...
            <Button variant="outline" size="sm" onClick={addCondition} className="justify-start">
            ...
            {conditions.length > 1 && (
                <>
                    <Separator />
                    <RadioGroup value={logic} onValueChange={(val) => setLogic(val as 'and' | 'or')} className="flex items-center gap-4">
...
function FilterInput({ columnType, condition, onChange } : {
    ...
    if (columnType === 'boolean') {
        return (
             <Select value={condition.value} onValueChange={val => onChange(condition.id, { value: val })}>
